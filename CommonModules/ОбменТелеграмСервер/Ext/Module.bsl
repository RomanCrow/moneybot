...
Процедура ОбработатьКоманды() Экспорт
	...
	Для Каждого Сообщение Из СтруктураОтвета.result Цикл
		...
		ИначеЕсли Команда = "ОперацииПоТоварам" Тогда
			...
			Если СтрНайти(ТекстСообщения, "добавить") <> 0 Или СтрНайти(ТекстСообщения, "новы") <> 0  Тогда
		...
		ИначеЕсли Команда = "РаспределениеЗатрат" Тогда
			
			РаспределитьЗатраты(ТекстСообщения, ТекстОтвета);
...
Функция ПолучитьОстатки(БезНал = Неопределено, Период = Неопределено)
...
Функция ОбработатьКомандыСообщения(ТекстСообщения)
	...
	ИначеЕсли СтрНайти(ТекстСообщения, "распредел") <> 0 Тогда
		СтруктураДанных.Операция = "РаспределениеЗатрат";
	КонецЕсли; 
...
Процедура ДобавитьТовары(ТекстСообщения, ТекстОтвета)
	...
	Статья = ПолучитьСтатьюПриходаРасхода(МассивДанных[0],"товар");
