...
Процедура ОбработатьКоманды() Экспорт
	...
	Для Каждого Сообщение Из СтруктураОтвета.result Цикл
		...
		ИначеЕсли Команда = "Приход" Тогда
			...
			ЭтоПлан = СтрНайти(ТекстСообщения,"план") <> 0;
		...
		ИначеЕсли Команда = "Расход" Тогда
			...
			Если Успех Тогда				
				ТекстОтвета = ТекстОтвета + ТекстОтветаУспешнаяОперация(Ложь, Сумма, Статья) + Символы.ПС + ТекстАнализЗатратЗаДень(Отправитель);							
			Иначе				
...
Функция ПолучитьПараметрыАнализа(ТекстСообщения)
	...
	СтруктураПараметров.Конец = ТекДата;
...
Процедура ВыполнитьПлановыеОперации(ТекстСообщения, ТипОперации, Пользователь, Сумма, Статья, ТекстОтвета)
	...
	ТекстОтвета = ТекстОтвета + "Операция прошла успешно. Ожидается зарплата " + Формат(Дата, "ДЛФ=DD") + " Сумма: " + Сумма + " руб." + Символы.ПС + ТекстСообщенияЗатратыВДень();
...
Функция ТекстСообщенияЗатратыВДень()
	//СуммаЗатрат = РасчетЗарплатыСервер.РасчитатьЗатратыДоЗарплаты();
	//Если СуммаЗатрат = Неопределено Тогда
	//	
	//	Возврат "Не планируется поступления зарплаты. Заполните плановые доходы.";
	//	
	//Иначе 
	//	
	//	Возврат "Максимальные затраты в день: " + Окр(СуммаЗатрат) + " руб. на всех, то есть в среднем " + Окр(СуммаЗатрат / КоличествоПользователей()) + " руб на одного.";
	//
	//КонецЕсли;
...
Функция ТекстАнализЗатратЗаДень(Пользователь)
	
	ТекДата = ТекущаяДатаСеанса();
	ПотраченоСегодня = ПотраченоЗаПериод(Пользователь, НачалоДня(ТекДата), КонецДня(ТекДата));
	НужноЗатратВДень = Окр(РасчетЗарплатыСервер.РасчитатьЗатратыДоЗарплаты(НачалоДня(ТекДата)) / КоличествоПользователей());
	
	ТекстВозврата = "Потрачено сегодня средств: " + ПотраченоСегодня + " руб.
					|Максимум затрат на сегодня: " + НужноЗатратВДень + " руб.";
	Если НужноЗатратВДень < ПотраченоСегодня Тогда
		ТекстВозврата = ТекстВозврата + Символы.ПС + "Вы уже потратили за сегодня больше, чем положено на " + (ПотраченоСегодня - НужноЗатратВДень) + " руб. Нельзя так...";
	ИначеЕсли НужноЗатратВДень = ПотраченоСегодня Тогда
		ТекстВозврата = ТекстВозврата + Символы.ПС + "Вы потратили за сегодня столько, сколько возможно. Постарайтесь завтра чуть-чуть побольше сэконосить...";
	Иначе
		ТекстВозврата = ТекстВозврата + Символы.ПС + "Ещё можно сегодня потратить " + НужноЗатратВДень - ПотраченоСегодня + " руб. Но лучше сэкономить:)";
	КонецЕсли; 
