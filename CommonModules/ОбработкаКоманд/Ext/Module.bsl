...
Процедура ОбработкаКоманды(ПараметрыКоманды) Экспорт
	...
	Если ПараметрыКоманды.Команда = Перечисления.Операции.ПоступлениеСредств Тогда
		ОформитьПоступлениеСредств(ПараметрыКоманды);
	КонецЕсли; 
...
Процедура ОформитьПоступлениеСредств(ПараметрыОперации)
	...
	Если ПараметрыОперации.ТипПланированияОперации = Перечисления.ТипПланированияОперации.ФактическаяОперация Тогда
		...
		СтруктураДокумента.Вставить("Пользователь", ПараметрыОперации.Отправитель);
		СтруктураДокумента.Вставить("ТипОперации", ПараметрыОперации.Команда);
		СтруктураДокумента.Вставить("Статья", ПараметрыОперации.Статья);  
		СтруктураДокумента.Вставить("Сумма", ПараметрыОперации.СуммыСообщения[0]);
		СтруктураДокумента.Вставить("Дата", ПараметрыОперации.ДатаСообщения);
		СтруктураДокумента.Вставить("ВидСредств", ПараметрыОперации.ВидСредств);
		
		Документы.Операция.СоздатьИЗаполнить(СтруктураДокумента);
		
		ПараметрыОперации.ТекстОтвета = ПараметрыОперации.ТекстОтвета + ТекстОтветаУспешнаяОперация(Истина, ПараметрыОперации.СуммыСообщения[0], ПараметрыОперации.Статья, Истина);
		
	Иначе	
		//ВыполнитьПлановыеОперации(ФорматированноеСообщение, Перечисления.ПриходРасход.Приход, Отправитель, Сумма, Статья, ПараметрыОперации.ТекстОтвета);
	КонецЕсли;  
