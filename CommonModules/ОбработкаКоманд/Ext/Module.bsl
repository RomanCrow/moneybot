...
Процедура ОбработкаКоманды(ПараметрыКоманды) Экспорт
	...
	ТекстСообщения = ПараметрыКоманды.ТекстСообщения;
	ФорматированноеСообщение = НРег(ПараметрыКоманды.ТекстСообщения);
	Отправитель = ПараметрыКоманды.Отправитель;
	СтруктураСообщения = ПараметрыКоманды.СтруктураСообщения;
	
	Сумма = СлужебныеПроцедурыКлиентСервер.ПолучитьЧислаИзСтроки(ФорматированноеСообщение)[0];
	Безнал = СтрНайти(ФорматированноеСообщение, "безнал") <> 0;
	
	Если Команда = Перечисления.Операции.ОперацииСФизЛицами Тогда
		
	    ВыполнитьОперацииСФизЛицами(ФорматированноеСообщение, ПараметрыКоманды.ТекстОтвета);
		
	ИначеЕсли Команда = Перечисления.Операции.ОбработкаНоменклатуры Тогда
		
		Если СтрНайти(ФорматированноеСообщение, "добавить") <> 0 Или СтрНайти(ФорматированноеСообщение, "новы") <> 0  Тогда
			ДобавитьТовары(ФорматированноеСообщение, ПараметрыКоманды.ТекстОтвета);
		КонецЕсли;
					
	ИначеЕсли Команда = Перечисления.Операции.ОбработкаЧека Тогда 	
		
		ПроверкаЧеков.ПроверитьЧек(ФорматированноеСообщение, Отправитель, ПараметрыКоманды.ТекстОтвета);
 						
	ИначеЕсли Команда = Перечисления.Операции.ОбработкаДолга Тогда
	...
	ИначеЕсли Команда = Перечисления.Операции.ПоступлениеСредств Тогда
	...
	ИначеЕсли Команда = Перечисления.Операции.КраткийОтчет Тогда
					
		ПараметрыКоманды.ТекстОтвета = ПараметрыКоманды.ТекстОтвета + ТекстОтветаУспешнаяОперация(,,,Ложь); 
		
	ИначеЕсли Команда = Перечисления.Операции.ПолныйОтчет Тогда
	...
	ИначеЕсли Команда = Перечисления.Операции.ПеремещениеСредств Тогда 	
	...
	ИначеЕсли Команда = Перечисления.Операции.РаспределениеЗатрат Тогда
...
Функция ПолучитьТекстДетальногоАнализаЗаПериод(Начало, Конец, Пользователь = Неопределено) Экспорт
	...
	ТекстСообщения = ТекстСообщения + "
		...
		|  * Наличные - " + ОбработкаКоманд.ПолучитьОстатки(Ложь) + " руб.
		|  * Безналичные - " + ОбработкаКоманд.ПолучитьОстатки(Истина) + " руб.";
...
Функция ТекстОтветаНедостаточноСредств(Безнал)
	...
	ТекстОтвета = "Недостаточно средств. У вас есть только " + ОбработкаКоманд.ПолучитьОстатки(Безнал) + " руб. " + ?(Безнал,"по безналу.","наличными.");
