...
Процедура ДобавитьУниграммыВСловарь(Строка) Экспорт
	...
	Сопоставления = СтрРазделить(Строка, ";");
	
	Если Сопоставления.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	
	Униграмма = СлужебныеПроцедурыКлиентСервер.ПолучитьУниграммуСтроки(Сопоставления[0]);
	...
	Для каждого Слово Из Униграмма Цикл
		Отбор.Наименование = Слово;
		НайденныеСтроки = Выгрузка.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() = 0 Тогда
			НовЭлемент = Справочники.Униграммы.СоздатьЭлемент();
			НовЭлемент.Наименование = Слово;
			НовЭлемент.Метафон = СлужебныеПроцедурыКлиентСервер.Metaphone(Слово);
			НовЭлемент.Записать();
		КонецЕсли; 		
	КонецЦикла; 
