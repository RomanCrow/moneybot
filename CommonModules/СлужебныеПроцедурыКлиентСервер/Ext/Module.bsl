...
Функция ПолучитьЧислаИзСтроки(Знач Строка) Экспорт
	...
	Для Инд = 1 По Длина Цикл
		...
		Если ЭтоЧисло Или (НачалоЧисла И (Символ = "." Или Символ = ",")) Тогда
...
Функция НайтиДатуВСтроке(Знач Строка) Экспорт
	
	ТекДата = ТекущаяДата();
	
	ЧислаИзСтроки = СлужебныеПроцедурыКлиентСервер.ПолучитьЧислаИзСтроки(Строка);
	Если (ЧислаИзСтроки.Количество() = 1 И ЧислаИзСтроки[0] = 0) Или ЧислаИзСтроки[0] <> Цел(ЧислаИзСтроки[0]) Тогда
		Возврат КонецДня(КонецДня(ТекДата) + 1);
	Иначе
		День = ЧислаИзСтроки[0];
	КонецЕсли;
