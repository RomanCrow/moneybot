Процедура ПроверитьЧек(Данные, Отправитель, СтрокаЛог) Экспорт
	...
	Запрос.УстановитьПараметр("Комментарий", Данные);
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		СтрокаЛог = "Этот чек уже обработан!";
		Возврат;
	КонецЕсли; 
	...
	МассивПараметров = СтрРазделить(Данные,"&",Ложь);
	...
	Если РезультатЗапроса.Пустой() Тогда
		СтрокаЛог = "Не заполнены настройки авторизации в ФНС!";
		Возврат;
	КонецЕсли; 
	...
	Если Ответ.КодСостояния <> 200 Тогда
		...
		Если Ответ.КодСостояния <> 200 Тогда
			СтрокаЛог = "Что-то пошло не так... ФНС не дает данные чека.";
		КонецЕсли;
	...
	Исключение
		СтрокаЛог = ОписаниеОшибки();
	    Возврат;
	КонецПопытки; 
	
	СтрокаЛог = ТекстВозвратаЧекОбработан(Док.Ссылка);
...
Функция ТекстВозвратаЧекОбработан(ДокПокупки)
	...
	Возврат Текст + ОбменТелеграмСервер.ТекстОтветаУспешнаяОперация(,,,Ложь);
